...
```{r}
data_all <- readr::read_csv("data.csv", col_types = "idicccddddcddciiiiiiddiiiiiiiddddddddddddddddddddddddcddddid") %>% 
  mutate(across(
    c(year,
      gender,
      relationship_status,
...
# B1
```{r}
model_B1 <- readRDS("./model_specifications/B1/model.rds")

vcov_B1 <- readRDS("./model_specifications/B1/vcov.rds")
...
# B2
```{r}
model_B2 <- readRDS("./model_specifications/B2/model.rds")

vcov_B2 <- readRDS("./model_specifications/B2/vcov.rds")
...
```{r}
model_B2_data_H1k_n <- readRDS("./model_specifications/B2_data_H1k-n/model.rds")

vcov_B2_data_H1k_n <- readRDS("./model_specifications/B2_data_H1k-n/vcov.rds")
...
```{r}
model_H1a <- readRDS("./model_specifications/H1a/model.rds")

vcov_H1a <- readRDS("./model_specifications/H1a/vcov.rds")
...
## H1b
```{r}
model_H1b <- readRDS("./model_specifications/H1b/model.rds")

vcov_H1b <- readRDS("./model_specifications/H1b/vcov.rds")
...
## H1c-f 
```{r}
model_H1c_f <- readRDS("./model_specifications/H1c_f/model.rds")

vcov_H1c_f <- readRDS("./model_specifications/H1c_f/vcov.rds")
...
```{r}
model_H1c_f_data_H1k_n <- readRDS("./model_specifications/H1c_f_data_H1k-n/model.rds")

vcov_H1c_f_data_H1k_n <- readRDS("./model_specifications/H1c_f_data_H1k-n/vcov.rds")
...
## H1g_j
```{r}
model_H1g_j <- readRDS("./model_specifications/H1g_j/model.rds")

vcov_H1g_j <- readRDS("./model_specifications/H1g_j/vcov.rds")
...
## H1k_n (Model 3)
```{r}
model_H1k_n <- readRDS("./model_specifications/H1k_n/model.rds")

vcov_H1k_n <- readRDS("./model_specifications/H1k_n/vcov.rds")
...
```{r}
data_H1k_n_countries <- model_H1k_n@frame %>% distinct(country_name, year) %>% mutate(covered = "X") %>% pivot_wider(names_from = year, values_from = covered, values_fill = "") %>% arrange(country_name) %>% mutate(country_name = as.character(country_name))

stargazer(data_H1k_n_countries, summary = FALSE, rownames = FALSE)
```
...
```{r}
model_B1_ML <- readRDS("./model_specifications/H1_ML/modelB1.rds")
model_B2_ML <- readRDS("./model_specifications/H1_ML/modelB2.rds")
model_H1c_f_ML <- readRDS("./model_specifications/H1_ML/modelH1c_f.rds")
model_H1g_j_ML <- readRDS("./model_specifications/H1_ML/modelH1g_j.rds")
model_H1k_n_ML <- readRDS("./model_specifications/H1_ML/modelH1k_n.rds")
...
ggsave(file = "figures/coeffient_sizes.pdf", width = 180, height = 223, units = "mm", device=cairo_pdf, create.dir = TRUE)
...
ggsave(file = "figures/effect_plot_gdp_median.pdf", width = 180, height = 130, units = "mm", device=cairo_pdf, create.dir = TRUE)
...
```{r}
new_data <- model_B2_data_H1k_n@frame %>% head(1) %>%  select(!c(life_evaluation, country_name, country_year)) %>% 
  rbind(list(year="2011",
  gdp = 7,
                      personal_income = 0.8477599*7,
                      age = 50,
                      age_squared = 50^2,
                      gender = "female",
                      relationship_status = "partnered",
                      growth = 0,
                      relative_income = 50
                     # food = 1, housing = 1, health = 1, water = 1, air = 1,
                     #             healthcare = 1, security = 1, social_support = 1, respect = 1,
                     #             education = 1, interesting_activity = 1, recreation = 1,
                     #             occupation = 1, freedom = 1,
                     #  country_average_food = 100,
                     #  country_average_housing = 100,
                     #  country_average_health = 85,
                     #  country_average_water = 100,
                     #  country_average_air = 100,
                     #  country_average_healthcare = 97,
                     #  country_average_security = 98,
                     #  country_average_social_support = 99,
                     #  country_average_respect = 99,
                     #  country_average_education = 99,
                     #  country_average_interesting_activity = 85,
                     #  country_average_recreation = 95,
                     #  country_average_occupation = 99,
                     #  country_average_freedom = 99,
                     #  democracy = max(data_all$democracy, na.rm = TRUE),
                     #  social_protection = max(data_all$social_protection, na.rm = TRUE),
                     #  government_effectiveness = max(data_all$government_effectiveness, na.rm = TRUE)
  )) 

X <- model.matrix(~year + gender + age + age_squared + relationship_status + personal_income + 
    relative_income + gdp + growth, data = new_data)
...
```{r}
effect_confidence_function <- function(model, age, gender, relationship_status, log_x = TRUE){
   ...
   effect <- function(x){
    ...
    new_data <-  
      model@frame %>% 
      head(1) %>% 
      select(!c(life_evaluation, country_name, country_year))
    
    new_data <- 
      new_data %>% 
      rbind(
        if(any(startsWith(colnames(new_data), "country_average"))){
          list(year="2011",
                gdp = x,
                personal_income = 0.8477599*x,
                age = age,
                age_squared = age^2,
                gender = gender,
                ...
                democracy = max(data_all$democracy, na.rm = TRUE),
                social_protection = max(data_all$social_protection, na.rm = TRUE),
                government_effectiveness = max(data_all$government_effectiveness, na.rm = TRUE))} else {
          if(any(startsWith(colnames(new_data), "food"))){
            list(year="2011",
                gdp = x,
                personal_income = 0.8477599*x,
                age = age,
  ...
  X <- model.matrix(colnames(new_data) %>% reformulate(), data = new_data)
    
   
  beta_hat <- fixef(model)
  V_beta <- as.matrix(vcov(model))
 
  for(year in 2010:2022){
  X[1,paste0("year", year)] <- 1/14
  }

  se <- sqrt(X %*% V_beta %*% t(X))


  mean <-  beta_hat %*% t(X)
  CI_low <-  mean - qnorm(1-0.05/2) * se
  CI_high <- mean + qnorm(1-0.05/2) * se
  
  return(data.frame(mean=c(mean),CI_low = c(CI_low), CI_high = c(CI_high)))
   }
  return(effect)
}
```
...
```{r}
data_effect_confidence_plot <-
  data.frame(gdp = seq(500,119500,1000)) %>%
  rowwise() %>% 
  mutate(B2_min = effect_confidence_function(
      model_B2_data_H1k_n, age = 50, #mean(data_all$age, na.rm=TRUE),
      gender = "male",
      relationship_status = "widowed"
      )(gdp),
      B2_max = effect_confidence_function(
      model_B2_data_H1k_n, age = 90, #mean(data_all$age, na.rm=TRUE),
      gender = "female",
      relationship_status = "partnered"
      )(gdp),
      H1c_f_min = effect_confidence_function(
      model_H1c_f_data_H1k_n, age = 50, #mean(data_all$age, na.rm=TRUE),
      gender = "male",
      relationship_status = "widowed"
        )(gdp),
      H1c_f_max =  effect_confidence_function(
      model_H1c_f_data_H1k_n, age = 90, #mean(data_all$age, na.rm=TRUE),
      gender = "female",
      relationship_status = "partnered"
      )(gdp),
      H1k_n_min =  effect_confidence_function(
      model_H1k_n, age = 50, #mean(data_all$age, na.rm=TRUE),
      gender = "male",
      relationship_status = "widowed"
      )(gdp),
      H1k_n_max = effect_confidence_function(
      model_H1k_n, age = 90, #mean(data_all$age, na.rm=TRUE),
      gender = "female",
      relationship_status = "partnered"
      )(gdp))

effect_confidence_plot <- 
  data_effect_confidence_plot %>% 
  ggplot(mapping = aes(x=gdp)) +
  geom_line(aes(y = B2_min$mean, col = "B2", linetype = "worst")) +
  geom_line(aes(y = B2_max$mean, col = "B2", linetype = "best")) +
  geom_ribbon(
    aes(
      ymin = B2_min$CI_low,
      ymax = B2_min$CI_high),
  ...
  geom_ribbon(
    aes(
      ...
      ymax = B2_max$CI_high),
    fill = "#4DBBD5FF", alpha = 0.3) +
  geom_line(aes(y = H1c_f_min$mean, col = "H1c_f", linetype = "worst")) +
  geom_line(aes(y = H1c_f_max$mean, col = "H1c_f", linetype = "best")) +
  geom_ribbon(
    aes(
      ymin = H1c_f_min$CI_low,
      ymax = H1c_f_min$CI_high),
    fill = "#F39B7FFF", alpha = 0.3) +
  geom_ribbon(
    aes(
      ymin = H1c_f_max$CI_low,
      ymax = H1c_f_max$CI_high),
    fill = "#F39B7FFF", alpha = 0.3) +
  geom_line(aes(y = H1k_n_min$mean, col = "H1k_n", linetype = "worst")) +
  geom_line(aes(y = H1k_n_max$mean, col = "H1k_n", linetype = "best")) +
  geom_ribbon(
    aes(
      ymin = H1k_n_min$CI_low,
      ymax = H1k_n_min$CI_high),
    fill = "#DC0000FF", alpha = 0.3) +
  geom_ribbon(
    aes(
      ymin = H1k_n_max$CI_low,
      ymax = H1k_n_max$CI_high),
    fill = "#DC0000FF", alpha = 0.3) +
  geom_rug(aes(x = jitter(exp(gdp))), data = data_subsample %>% filter(year==2022), linewidth = 0.1) +
  geom_rug(aes(x = jitter(exp(gdp))), data = data_subsample %>% filter(year==2022), linewidth = 0.1, color = "grey", sides = "t") +
  scale_colour_manual(name = NULL,
...
ggsave(file = "figures/effect_plot_gdp_median_confidence.pdf", width = 180, height = 130, units = "mm", device=cairo_pdf)
...
## H2a
```{r}
model_H2a <- readRDS("./model_specifications/H2a/model.rds")




vcov_H2a <- readRDS("./model_specifications/H2a/vcov.rds")
...
ggsave(file = "figures/H2a_conditional_effect_growth_protection.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)
```
```{r}
...
## H2b
```{r}
model_H2b <- readRDS("./model_specifications/H2b/model.rds")




vcov_H2b <- readRDS("./model_specifications/H2b/vcov.rds")
...
#ggsave(file = "figures/H2b_conditional_effect_gdp_government_effectiveness.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)
```
...
```{r}
ggintplot(model_H2b_lme4, varnames = c("personal_income", "government_effectiveness"), 
          vcov = vcov_H2b,
...
```
```{r}
ggintplot(model_H2b_lme4, varnames = c("relative_income", "government_effectiveness"), 
          vcov = vcov_H2b,
...
## H2c
```{r}
model_H2c <- readRDS("./model_specifications/H2c/model.rds")




vcov_H2c <- readRDS("./model_specifications/H2c/vcov.rds")
...
ggsave(file = "figures/H2c_conditional_effect_gdp_democracy_index.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)

```
```{r}
ggintplot(obj = model_H2c_lme4, varnames = c("democracy", "gdp"), 
          vcov = vcov_H2c,
...
## H2d
```{r}
model_H2d <- readRDS("./model_specifications/H2d/model.rds")




vcov_H2d <- readRDS("./model_specifications/H2d/vcov.rds")
...
#ggsave(file = "figures/H2d_conditional_effect_gdp_importance_rich.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)
```

```{r}
ggintplot(obj = model_H2d_lme4, varnames = c("growth", "importance_rich"), 
          vcov = vcov_H2d,
...
```
```{r}
ggintplot(obj = model_H2d_lme4, varnames = c("relative_income", "importance_rich"), 
          vcov = vcov_H2d,
...
```{r}
model_H3 <- readRDS("./model_specifications/H3/model.rds")

vcov_H3 <- readRDS("./model_specifications/H3/vcov.rds")
...
## H3b
```{r}
ggintplot(obj = model_H3_lme4, varnames = c("gendermale", "relative_income"), 
          vcov = vcov_H3,
...
ggsave(file = "figures/H3b_conditional_effect_gender_relative_income.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)
...
ggsave(file = "figures/H3c_conditional_effect_education_personal_income.pdf", width = 128, height = 65, units = "mm", device=cairo_pdf, create.dir = TRUE)
```
```{r}
...
ggsave(file = "figures/H3c_conditional_effect_education_relative_income.pdf", width = 128, height = 65, units = "mm", device=cairo_pdf, create.dir = TRUE)
```
```{r}
...
ggsave(file = "figures/H3c_conditional_effect_education_growth_for_mulitplot.pdf", width = 80, height = 50, units = "mm", device=cairo_pdf, create.dir = TRUE)
```
## H3d
...
ggsave(file = "figures/H3d_conditional_effect_relative_income_altruistic_behaviour.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)
...
## H4a1
```{r}
model_H4a1_med <- readRDS("./model_specifications/H4a1/modelmed.rds")

vcov_H4a1_med <- readRDS("./model_specifications/H4a1/vcovmed.rds")

model_H4a1_out <- readRDS("./model_specifications/H4a1/modelout.rds")

vcov_H4a1_out <- readRDS("./model_specifications/H4a1/vcovout.rds")
...
## H4a2
```{r}
model_H4a2_med <- readRDS("./model_specifications/H4a2/modelmed.rds")

vcov_H4a2_med <- readRDS("./model_specifications/H4a2/vcovmed.rds")

model_H4a2_out <- readRDS("./model_specifications/H4a2/modelout.rds")

vcov_H4a2_out <- readRDS("./model_specifications/H4a2/vcovout.rds")
```

## H4b
```{r}
model_H4b_med <- readRDS("./model_specifications/H4b/modelmed.rds")

vcov_H4b_med <- readRDS("./model_specifications/H4b/vcovmed.rds")

model_H4b_out <- readRDS("./model_specifications/H4b/modelout.rds")

vcov_H4b_out <- readRDS("./model_specifications/H4b/vcovout.rds")
```
...
```{r}
sobel_test("Hypothesis" = "H4-b", "model_med" = model_H4b_med, "vcov_med" = vcov_H4b_med, "model_out" = model_H4b_out, "vcov_out" = vcov_H4b_out, model_orig = model_H1k_n, vcov_orig = vcov_H1k_n,
           "predictor" = "growth", "mediator" = "expected_economic_conditions", "Predictor" = "GDP growth", "Mediator" = "Expected economic conditions", method = "Satterthwaite", decimal_places = 20)
```
...
## H4c_d
```{r}
model_H4c_d_med <- readRDS("./model_specifications/H4c_d/modelmed.rds")

vcov_H4c_d_med <- readRDS("./model_specifications/H4c_d/vcovmed.rds")

model_H4c_d_out <- readRDS("./model_specifications/H4c_d/modelout.rds")

vcov_H4c_d_out <- readRDS("./model_specifications/H4c_d/vcovout.rds")
```
...
```{r}
models_needs <- readRDS("./model_specifications/Exp_needs_as_outcomes/models_needs.rds")
```
...
ggsave(file = "figures/scatter_plot_needs_gdp_connected_highlighted.pdf", width = 180, height = 160, units = "mm", device=cairo_pdf, create.dir = TRUE)
```
...
## B2
```{r}
model_Exp_gini_B2 <- readRDS("./model_specifications/Exp_gini_B2/model.rds")

vcov_Exp_gini_B2 <- readRDS("./model_specifications/Exp_gini_B2/vcov.rds")

model_Exp_gini_B2@vcov_beta <- as.matrix(vcov_Exp_gini_B2) #replace vcov by cluster robust version


```

```{r}
summary(model_Exp_gini_B2)
```

```{r}
p_values_model_Exp_gini_B2 <- summary(model_Exp_gini_B2, ddf="Satterthwaite")$coefficients[,"Pr(>|t|)"]
coef_model_Exp_gini_B2 <- summary(model_Exp_gini_B2, ddf="Satterthwaite")$coefficients[,"Estimate"]
```

## H1c-f
```{r}
model_Exp_gini_H1c_f <- readRDS("./model_specifications/Exp_gini_H1c_f/model.rds")

vcov_Exp_gini_H1c_f <- readRDS("./model_specifications/Exp_gini_H1c_f/vcov.rds")

model_Exp_gini_H1c_f@vcov_beta <- as.matrix(vcov_Exp_gini_H1c_f) #replace vcov by cluster robust version


```

```{r}
summary(model_Exp_gini_H1c_f)
```

```{r}
p_values_model_Exp_gini_H1c_f <- summary(model_Exp_gini_H1c_f, ddf="Satterthwaite")$coefficients[,"Pr(>|t|)"]
coef_model_Exp_gini_H1c_f <- summary(model_Exp_gini_H1c_f, ddf="Satterthwaite")$coefficients[,"Estimate"]
```
...
## H1g-j
```{r}
model_Exp_gini_H1g_j <- readRDS("./model_specifications/Exp_gini_H1g_j/model.rds")

vcov_Exp_gini_H1g_j <- readRDS("./model_specifications/Exp_gini_H1g_j/vcov.rds")

model_Exp_gini_H1g_j@vcov_beta <- as.matrix(vcov_Exp_gini_H1g_j) #replace vcov by cluster robust version


```

```{r}
summary(model_Exp_gini_H1g_j)
```

## H1k-n
```{r}
model_Exp_gini_H1k_n <- readRDS("./model_specifications/Exp_gini_H1k_n/model.rds")

vcov_Exp_gini_H1k_n <- readRDS("./model_specifications/Exp_gini_H1k_n/vcov.rds")

model_Exp_gini_H1k_n@vcov_beta <- as.matrix(vcov_Exp_gini_H1k_n) #replace vcov by cluster robust version


```

```{r}
summary(model_Exp_gini_H1k_n)
```
```{r}
p_values_model_Exp_gini_H1k_n <- summary(model_Exp_gini_H1k_n, ddf="Satterthwaite")$coefficients[,"Pr(>|t|)"]
coef_model_Exp_gini_H1k_n <- summary(model_Exp_gini_H1k_n, ddf="Satterthwaite")$coefficients[,"Estimate"]
```
### Stargazer

```{r}
model_Exp_gini_B2_lme4 <- as.lmerMod.lmerModLmerTest(model_Exp_gini_B2)
model_Exp_gini_H1c_f_lme4 <- as.lmerMod.lmerModLmerTest(model_Exp_gini_H1c_f)
model_Exp_gini_H1k_n_lme4 <- as.lmerMod.lmerModLmerTest(model_Exp_gini_H1k_n)
stargazer(model_Exp_gini_B2_lme4, model_Exp_gini_H1c_f_lme4, model_Exp_gini_H1k_n_lme4, 
          #type = "text", 
          report = 'vc*', 
          omit = c("year*"),
          coef = list(coef_model_Exp_gini_B2, coef_model_Exp_gini_H1c_f, coef_model_Exp_gini_H1k_n),
          p = list(p_values_model_Exp_gini_B2, p_values_model_Exp_gini_H1c_f, p_values_model_Exp_gini_H1k_n),
          se = list(sqrt(diag(vcov_Exp_gini_B2)), sqrt(diag(vcov_Exp_gini_H1c_f)), sqrt(diag(vcov_Exp_gini_H1k_n))), 
          intercept.bottom = FALSE, 
          align = TRUE, 
          dep.var.caption = "",
          dep.var.labels.include = FALSE,
          no.space = TRUE,
          title = "Multilevel model coefficients when including the Gini coefficient",
          dep.var.labels = "Positive affect",
          order = paste0("^", c("Constant", "gendermale", "age", "age_squared", "relationship_statuspartnered", "relationship_statusseparated",
                    "relationship_statuswidowed", "food1", "housing1", "health1", "water1", "air1",
                    "healthcare1", "security1", "social_support1", "respect1", "education1",
                    "interesting_activity1", "recreation1", "occupation1", "freedom1",
 "country_average_food", "country_average_housing", "country_average_health", 'country_average_water', 
                    ...
                    "country_average_social_support", "country_average_respect", "country_average_education",
                    "country_average_interesting_activity", "country_average_recreation", "country_average_occupation",
                    "country_average_freedom", "government_effectiveness", "democracy", "social_protection", "personal_income", "relative_income", "gdp", "growth", "gini") , "$"),
          covariate.labels = c("Intercept", "Male", "Age", "Age (squared)", "Partnered", "Separated", "Widowed", "Food", "Housing", "Health", "Water", "Air", "Healthcare", "Security", "Social support", "Respect", "Education", "Interesting activity", "Recreation", "Occupation", "Freedom", "Food (country average)", "Housing (country average)", "Health (country average)", "Water (country average)", "Air (country average)", "Healthcare (country average)", "Security (country average)", "Social support (country average)", "Respect (country average)", "Education (country average)", "Interesting activity (country average)", "Recreation (country average)", "Occupation (country average)", "Freedom (country average)",  "Government effectiveness", "Democracy", "Social protection", "Log(personal income)", "Relative income", "Log(GDP per capita)", "GDP per capita growth", "Gini coefficient"),
          keep.stat = c("n", "ser"),
          add.lines = list(c("Residual standard deviation", 
                             signif(sigma(model_Exp_gini_B2),4),
                             signif(sigma(model_Exp_gini_H1c_f),4),
                             signif(sigma(model_Exp_gini_H1k_n),4)),
                           c("Marginal Pseudo-$R^2$", 
                              signif(MuMIn::r.squaredGLMM(model_Exp_gini_B2)[[1,"R2m"]],4),
                              signif(MuMIn::r.squaredGLMM(model_Exp_gini_H1c_f)[[1,"R2m"]],4),
                             signif(MuMIn::r.squaredGLMM(model_Exp_gini_H1k_n)[[1,"R2m"]],4)),
                            c("Conditional Pseudo-$R^2$", 
                              signif(MuMIn::r.squaredGLMM(model_Exp_gini_B2)[[1,"R2c"]],4),
                              signif(MuMIn::r.squaredGLMM(model_Exp_gini_H1c_f)[[1,"R2c"]],4),
                              signif(MuMIn::r.squaredGLMM(model_Exp_gini_H1k_n)[[1,"R2c"]],4))),
          star.char = c("+","*","**","***"),
          star.cutoffs =c(0.1, 0.05, 0.01, 0.001), 
          notes = c("$+$ p$<$0.1; $^*$ p$<$0.05; $^{**}$ p$<$0.01; $^{***}$ p$<$0.001"),
          notes.append = F,
          label = "tab:Exp_gini_B2_Hc_f")
...
## B2
```{r}
model_Exp_ls_B2 <- readRDS("./model_specifications/Exp_ls_B2/model.rds")

vcov_Exp_ls_B2 <- readRDS("./model_specifications/Exp_ls_B2/vcov.rds")

model_Exp_ls_B2@vcov_beta <- as.matrix(vcov_Exp_ls_B2) #replace vcov by cluster robust version



model_Exp_le_B2 <- readRDS("./model_specifications/Exp_le_data_ls_B2/model.rds")

vcov_Exp_le_B2 <- readRDS("./model_specifications/Exp_le_data_ls_B2/vcov.rds")
...
```{r}
model_Exp_ls_H1c_f <- readRDS("./model_specifications/Exp_ls_H1c_f/model.rds")

vcov_Exp_ls_H1c_f <- readRDS("./model_specifications/Exp_ls_H1c_f/vcov.rds")
...
model_Exp_le_H1c_f <- readRDS("./model_specifications/Exp_le_data_ls_H1c_f/model.rds")

vcov_Exp_le_H1c_f <- readRDS("./model_specifications/Exp_le_data_ls_H1c_f/vcov.rds")
...
```{r}
vcov_Exp_obj_H1a <- readRDS("./model_specifications/Exp_obj_H1a/vcov.rds")

model_Exp_obj_H1a <- readRDS("./model_specifications/Exp_obj_H1a/model.rds")
...
vcov_Exp_obj_H1b <- readRDS("./model_specifications/Exp_obj_H1b/vcov.rds")

model_Exp_obj_H1b <- readRDS("./model_specifications/Exp_obj_H1b/model.rds")
...
vcov_Exp_obj_H1c_f <- readRDS("./model_specifications/Exp_obj_H1c_f/vcov.rds")

model_Exp_obj_H1c_f <- readRDS("./model_specifications/Exp_obj_H1c_f/model.rds")
...
vcov_Exp_obj_H1g_j <- readRDS("./model_specifications/Exp_obj_H1g_j/vcov.rds")

model_Exp_obj_H1g_j <- readRDS("./model_specifications/Exp_obj_H1g_j/model.rds")
...
vcov_Exp_obj_H1k_n <- readRDS("./model_specifications/Exp_obj_H1k_n/vcov.rds")

model_Exp_obj_H1k_n <- readRDS("./model_specifications/Exp_obj_H1k_n/model.rds")
...
stargazer(model_B2_lme4, model_Exp_obj_H1c_f_lme4,
          #type = "text", 
          report = 'vc*', 
          omit = c("year*"),
          coef = list(coef_model_B2, coef_model_Exp_obj_H1c_f),
          p = list(p_values_model_B2, p_values_model_Exp_obj_H1c_f),
          se = list(sqrt(diag(vcov_B2)), sqrt(diag(vcov_Exp_obj_H1c_f))), 
          intercept.bottom = FALSE, 
          align = TRUE, 
          dep.var.caption = "",
          dep.var.labels.include = FALSE,
...
### H2a
```{r}
model_Exp_obj_H2a <- readRDS("./model_specifications/Exp_obj_H2a/model.rds")

vcov_Exp_obj_H2a <- readRDS("./model_specifications/Exp_obj_H2a/vcov.rds")
...
### H2b
```{r}
model_Exp_obj_H2b <- readRDS("./model_specifications/Exp_obj_H2b/model.rds")


model_Exp_obj_H2b <- readRDS("./model_specifications/Exp_obj_H2b/model.rds")

vcov_Exp_obj_H2b <- readRDS("./model_specifications/Exp_obj_H2b/vcov.rds")
...
### H2c
```{r}
model_Exp_obj_H2c <- readRDS("./model_specifications/Exp_obj_H2c/model.rds")



vcov_Exp_obj_H2c <- readRDS("./model_specifications/Exp_obj_H2c/vcov.rds")
...
#ggsave(file = "figures/Exp_obj_H2c_conditional_effect_gdp_democracy_index.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)
...
model_Exp_obj_H2d <- readRDS("./model_specifications/Exp_obj_H2d/model.rds")



vcov_Exp_obj_H2d <- readRDS("./model_specifications/Exp_obj_H2d/vcov.rds")
...
#ggsave(file = "figures/Exp_obj_H2d_conditional_effect_gdp_importance_rich.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)
```
...
```{r}
model_Exp_obj_H3 <- readRDS("./model_specifications/Exp_obj_H3/model.rds")

vcov_Exp_obj_H3 <- readRDS("./model_specifications/Exp_obj_H3/vcov.rds")
...
ggsave(file = "figures/Exp_obj_H3d_conditional_effect_GDP_altruistic_behaviour.pdf", width = 160, height = 110, units = "mm", device=cairo_pdf, create.dir = TRUE)
...
### H4a1
```{r}
model_Exp_obj_H4a1_med <- readRDS("./model_specifications/Exp_obj_H4a1/modelmed.rds")

vcov_Exp_obj_H4a1_med <- readRDS("./model_specifications/Exp_obj_H4a1/vcovmed.rds")

model_Exp_obj_H4a1_out <- readRDS("./model_specifications/Exp_obj_H4a1/modelout.rds")

vcov_Exp_obj_H4a1_out <- readRDS("./model_specifications/Exp_obj_H4a1/vcovout.rds")
...
### H4a2
```{r}
model_Exp_obj_H4a2_med <- readRDS("./model_specifications/Exp_obj_H4a2/modelmed.rds")

vcov_Exp_obj_H4a2_med <- readRDS("./model_specifications/Exp_obj_H4a2/vcovmed.rds")

model_Exp_obj_H4a2_out <- readRDS("./model_specifications/Exp_obj_H4a2/modelout.rds")

vcov_Exp_obj_H4a2_out <- readRDS("./model_specifications/Exp_obj_H4a2/vcovout.rds")
```

### H4b
```{r}
model_Exp_obj_H4b_med <- readRDS("./model_specifications/Exp_obj_H4b/modelmed.rds")

vcov_Exp_obj_H4b_med <- readRDS("./model_specifications/Exp_obj_H4b/vcovmed.rds")

model_Exp_obj_H4b_out <- readRDS("./model_specifications/Exp_obj_H4b/modelout.rds")

vcov_Exp_obj_H4b_out <- readRDS("./model_specifications/Exp_obj_H4b/vcovout.rds")
```
...
### H4c_d
```{r}
model_Exp_obj_H4c_d_med <- readRDS("./model_specifications/Exp_obj_H4c_d/modelmed.rds")

vcov_Exp_obj_H4c_d_med <- readRDS("./model_specifications/Exp_obj_H4c_d/vcovmed.rds")

model_Exp_obj_H4c_d_out <- readRDS("./model_specifications/Exp_obj_H4c_d/modelout.rds")

vcov_Exp_obj_H4c_d_out <- readRDS("./model_specifications/Exp_obj_H4c_d/vcovout.rds")
```
...
mediation_results <- mediation_table(mediation_models, stars = TRUE, method = "Satterthwaite", model_orig = model_Exp_obj_H1k_n, vcov_orig = vcov_Exp_obj_H1k_n)
